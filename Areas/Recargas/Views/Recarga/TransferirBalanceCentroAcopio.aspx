...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h2>
        Transferir Balance a Centros de Acopio
    </h2>
    <form id="frmTransf" action="/Recargas/Recarga/TransferirBalanceCentroAcopio" onsubmit="return postData()" method="post">
    <b>Balance en Consorcio</b>
    ...
    <table id="tbc<%=tableCount++ %>" class="table" style="width: 500px">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    Transferido
                    <input type="hidden" id="RecargaId<%=rowCount%>" name="Balances[<%=rowCount%>].RecargaId"
                        value="<%=balance.Id %>" />
                </td>
                <td class="monto">
                    <input id="Claro<%=rowCount%>" name="Balances[<%=rowCount%>].Claro" type="text" <% if(Model.AvailableBalance.ClaroBalance > 0)
                       {%> onblur="calc(this)" <% } else {%> disabled="disabled" style="background-color: #cccccc"
                        <% }%> />
                </td>
                <td class="monto">
                ...
                <td class="monto">
                    <input id="Viva<%=rowCount%>" name="Balances[<%=rowCount%>].Viva" type="text" <% if(Model.AvailableBalance.VivaBalance > 0)
                       {%> onblur="calc(this)" <% } else {%> disabled="disabled" style="background-color: #cccccc"
                        <% }%> />
                </td>
                <td class="monto">
                    <input id="Tricom<%=rowCount %>" name="Balances[<%=rowCount%>].Tricom" type="text" <% if(Model.AvailableBalance.TricomBalance > 0)
                       {%> onblur="calc(this)" <% } else {%> disabled="disabled" style="background-color: #cccccc"
                        <% }%> />
                </td>
                <td class="monto">
    ...
    <%
        ...
        {
    %>
    <b>Centros de Acopio sin balance definido</b>
    ...
    <div id="msgInvalidMonto" style="font-size: 12pt; color: red; display: none; padding: 15px;">
        <p>
            Los montos suministrados exceden el balance disponible.</p>
        <br />
    </div>
    <table style="width: 500px; padding-bottom: 15px">
        <tr>
            <td>
                <input id="acceptTransfer" type="checkbox" onclick="calc()" /><label for="acceptTransfer">He
                    revisado los datos</label>
            </td>
            <td  style="text-align: right">
                <input id="btnTransf" type="submit" value="Transferir Balance" />
            </td>
        </tr>
    </table>
    </form>
...
<asp:Content ID="Content3" ContentPlaceHolderID="Assets" runat="server">
    ...
    <script type="text/javascript">
        ...
        function postData() {
            ...
            $.ajax({
                type: "POST",
                url: "/Recargas/Recarga/TransferirBalanceCentroAcopio",
                data:  $("#frmTransf").serialize(),
                dataType: "text",
